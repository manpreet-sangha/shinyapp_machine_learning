...
| **Random Forest** | Configurable trees, depth, and test size (sliders at top). Sub-tabs: *Understanding Random Forests* (Bagging 4-step process, OOB error, max_features decorrelation), *Feature Importance* (Gini-based, scaled 0–100, with explanation card), *Performance* (full metrics + confusion matrix), *ROC Curve* (with layman explanation), *Trees vs Error* (test error + OOB error lines), *max_features (m)* (compares m = p, p/2, √p) |
| **Gradient Boosting** | Tunable stages, depth, learning rate, and test size (sliders at top). Sub-tabs: *Understanding Gradient Boosting* (concept overview + hyperparameter guide), *Feature Importance* (scaled 0–100, amber bars), *Performance* (full metrics + confusion matrix), *ROC Curve* (with layman explanation), *Staged Accuracy* |
